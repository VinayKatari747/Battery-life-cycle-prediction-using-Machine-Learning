...
for i = 1:46
    ...
    % Use the 'line' function to specify individual line segment colors
    for j = 1:(k(i)-1)
        line([runningAvg(j), runningAvg(j+1)], [V(j), V(j+1)], 'Color', lineColor(j, :), 'DisplayName', ['Cell ', num2str(i)]);
    end
end
